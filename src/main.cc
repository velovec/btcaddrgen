...
int main(int argc, const char *argv[]) {
  ...
  for (int i = 0; i < 6000; i++ ) {
...

    {
      ...
      utils::die_on_error(amqp_basic_publish(conn, 1, amqp_cstring_bytes(exchange), amqp_cstring_bytes(routingkey), 0, 0, &props, amqp_cstring_bytes(addrString)), "Publishing");
    }
  }
